...
const movedImg = (e)=>{
  ...
  $('.box3-right').css({right:-50+x/60,bottom:-50-y/60});

  $('.box4-left').css({left:-200+x/50,top:50-y/50});
  $('.box4-right').css({right:-50+x/20,bottom:-50+y/20});
...
// 로고 컬러 변경
$(window).on('scroll',()=>{
  const box1P=$('.txt-wrapper p').offset().top;
  console.log(box1P);
/*   if(scrollY>box1P){
  ...
  $('header').toggleClass('colorChange',scrollY>box1P);

 // 해당 박스로 이동하면 내비에 .active 붙음
 const idx=Math.floor(scrollY/$(window).height());
  $('.slide-menu li').removeClass('active').eq(idx).addClass('active');
...
$('.slide-menu li').on('click',(e)=>{
 ...
 /*  scrollTo({ -> px 반환 받음
  ...
  $('html, body').stop().animate({scrollTop:currentBox},800,'easeInOutExpo');
